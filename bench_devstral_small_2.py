"""
One-off benchmark script: devstral-small-2 vs. the current granite4:tiny-h
daily driver (promoted 2026-09-16, replacing qwen3-coder:30b). Last item
in the approved queue. Run once from AI-Server, results logged into
Active Priorities / This AI-Server by hand afterward - same methodology
as every prior model bench on this project: real head-to-head, no
trusting a leaderboard number.

Uses /api/chat with think:false throughout (lesson from the granite4.2:8b
bench 2026-09-16: /api/generate skips the chat template and leaks raw
reasoning into the answer regardless of the think flag).
...
bench alone, without paying for the full fabrication/tool run too:
    ...
    python bench_devstral_small_2.py coding
    python bench_devstral_small_2.py tools
    python bench_devstral_small_2.py fabrication
"""
import json
import subprocess
import sys
